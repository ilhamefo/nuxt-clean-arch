...
watch(() => formData.value.level, (newLevel, oldLevel) => {
    if (newLevel !== null && newLevel !== undefined) {
        userStore.getUnitsNew(`${newLevel}`)
    }
})
...
watch(selectedUser, async (newUser) => {
    if (newUser) {
        ...
        if (newUser.level !== null && newUser.level !== undefined) {
            try {
                await userStore.getUnitsNew(`${newUser.level}`)
...
const onSubmit = async () => {
    ...
    try {
        const response = await userStore.updateNew(selectedUser.value.id, {
...
const onSyncSearchData = async () => {
    ...
    try {
        const response = await userStore.syncSearchDataNew();
        if (response && response.status === 200) {
            snackbar.add({
                type: 'success',
